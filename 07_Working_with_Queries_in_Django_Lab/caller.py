...
def filter_authors_by_nationalities(nationality):
    ...
    output = [f'{author.biography}' if author.biography else f'{author.first_name} {author.last_name}'
...
def filter_authors_by_birth_year(first_year, second_year):
    ...
    searched_authors_info = [
        ...
        for author in Author.objects.filter(
            birth_date__year__gte=first_year, birth_date__year__lte=second_year
        ).order_by('-birth_date')
    ]
